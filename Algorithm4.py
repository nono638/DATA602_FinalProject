...
if __name__ == '__main__':
    ...
    Sourcedf = NoahAndChrisTesting.ChrisDF
    Stenographer = "Chris"
...
#TODO loop through temp dicts if applicable, has to deal with breaking up consonant batches.
                ...
                for tempBatch in tempBatches:
                    ...
                    if runningSteno != None:
                        if highScorer != None:
                            if side == 'right': #This is my terrible attempt at multi-stroke steno
                                runningSteno += highScorer + " "
                            elif batchNum > 0  and side == 'left':
                                if type(runningSteno) is str and len(runningSteno)>0 and  runningSteno[-1] != " ":
                                    runningSteno +=  " "
                                runningSteno += highScorer
                            elif previousSide == 'right' and side == 'mid':
                                if type(runningSteno) is str and len(runningSteno)>0 and runningSteno[-1] != " ":
                                    runningSteno += " "
                                runningSteno += highScorer
                            else:
    ...
    dfPresentable = df.copy(deep = True)
    dfPresentable = dfPresentable[["Steno", 'Word', 'SyllableGuess',\
                                   'SyllableGuessCorrect',
                                   "PhoneticGuesses", 'PhoneticGuessCorrect',
                                   "NumStrokesCorrect"]]
